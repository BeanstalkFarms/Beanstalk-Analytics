  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        const apiResp = await fetch(reqUrlApi.toString(), {"headers": headers }).then(r => r.json()); 
        ...
        const { schema, timestamp } = await fetch(reqUrlBucket.toString(), {"headers": headers }).then(r => r.json()); 
        setSpec(schema); 
        setStatus('ready');
      } catch(e) {
        console.error(e);
